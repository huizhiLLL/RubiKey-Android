...
- `CubeMove`：Moyu32 可报告的 12 种单步正反转动及显示记号。
...
单元测试覆盖 12 种编号转换、未知编号拒绝、Provider 无匹配和多重匹配错误。
...
删除 `MainActivity`、计时、打乱、3D 和陀螺仪 UI 依赖。不发送 `172` 陀螺仪启用请求，收到 `171/172` 陀螺仪相关报文时直接忽略；其他结果改由 `CubeEventSink` 输出。原始转动在协议层转换为 `CubeMove`。
...
- `ActionMapping`：12 种单步正反转动到动作的完整映射。
...
动作编辑使用 Material 3 底部面板：可选择无动作、点击或滑动；点击使用竖屏比例坐标画布，滑动只提供上、下、左、右预设。界面不使用悬浮窗录制，不堆叠装饰卡片，不用颜色替代文字状态。
...
2. 扫描并连接 Moyu32，连续执行 12 种单步正反转动，确认标准事件和映射一致。
